...
from . import utils_intel as utils
...
# from cuml.preprocessing import StandardScaler   #run GPU version
from sklearn.preprocessing import StandardScaler   #run CPU version
from torch.utils.data import TensorDataset, DataLoader    
from . import trpca_intel as trpca
...
def trpca_regress(feature_x,label_y, age_bins,test_size=0.2, n_dimensions=128, num_transformer_layers=1, nhead=8, dim_feedforward=2048, epochs=1000, learning_rate=1e-04,wt_decay=0.1,batch_size=512):


    # Splitting the dataset
    X_train, X_test, y_train, y_test = train_test_split(feature_x, label_y, test_size=test_size, random_state=40,shuffle=True)
    ...
    train_features = torch.as_tensor(X_train_scaled, dtype=torch.float32)
    test_features = torch.as_tensor(X_test_scaled, dtype=torch.float32)
    train_targets = torch.as_tensor(y_train, dtype=torch.float32).unsqueeze(1)
    test_targets = torch.as_tensor(y_test, dtype=torch.float32).unsqueeze(1)
    ...
    train_loader = DataLoader(train_dataset,num_workers=0, batch_size=batch_size, shuffle=False)
    ...
    optimizer = torch.optim.SGD(regression_model.parameters(), lr=learning_rate, weight_decay=wt_decay)
    ...
    for epoch in tqdm(range(epochs), total=epochs):
        ...
        for batch_features, batch_labels in train_loader:
            ...
            with torch.cpu.amp.autocast():
                ...
                loss.backward()                   #training optimizarion using intel extension for pytorch , could try to put other parts in autocast as well
        ...
        with torch.no_grad():
            for batch_features, batch_labels in test_loader:
                batch_features, batch_labels = batch_features.to(device), batch_labels.to(device)
                with torch.cpu.amp.autocast():    #for different data type , BF16, FP32
                    outputs = regression_model(batch_features)
                    loss = criterion(outputs['regression_output'], batch_labels)    #valid optimizarion using intel extension for pytorch , could try to put other parts in autocast as well
                    #tunable base on the machine performance
        ...
        if valid_loss_epoch < best_valid_loss:
            ...
            # Save the model
            if epoch>epochs/2:
                torch.save(regression_model.state_dict(), '/home/vaquita/multiomic/best_designated_skin_age_regression_model.pth')
            # print(f'Epoch {epoch+1}: Validation loss improved, model saved.')

    # Close the progress bar after all epochs are done
    pbar.close()
    regression_model.load_state_dict(torch.load('/home/vaquita/multiomic/best_designated_skin_age_regression_model.pth'))
    ...
    plt.legend()
    plt.savefig('training_progress1.jpg')
    return scaler
...
def trpca_classify(table, metadata, MetadataColumn, test_size=0.2, n_dimensions=128, feature_frequency=5, num_transformer_layers=1, nhead=8, dim_feedforward=2048, epochs=1000):
    ...
    plt.savefig('training_progress2.jpg')
    ...
    plt.title('Training and Validation Loss Over Epochs')
    plt.legend()
    plt.savefig('training_progress3.jpg')
    ...
    plt.tight_layout()
    plt.savefig('training_progress4.jpg')
